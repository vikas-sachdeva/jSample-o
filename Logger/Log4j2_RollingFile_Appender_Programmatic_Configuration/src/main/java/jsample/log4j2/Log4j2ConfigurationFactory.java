...
public class Log4j2ConfigurationFactory extends ConfigurationFactory {
	...
	private static Configuration createConfiguration(String name, ConfigurationBuilder<BuiltConfiguration> builder) {
		...
		AppenderComponentBuilder appenderBuilder = builder.newAppender("fileLogger", RollingFileAppender.PLUGIN_NAME)
		...
		appenderBuilder = builder.newAppender("consoleLogger", ConsoleAppender.PLUGIN_NAME).addAttribute("target",
		...
		builder.add(builder.newLogger("jsample", Level.INFO).add(builder.newAppenderRef("fileLogger"))
				.add(builder.newAppenderRef("consoleLogger")).addAttribute("additivity", false));

		builder.add(builder.newRootLogger(Level.ERROR).add(builder.newAppenderRef("fileLogger"))
				.add(builder.newAppenderRef("consoleLogger")).addAttribute("additivity", false));
